...
const getSubcategoryTags = (tree, tags) => {
  if (tree.children && Object.keys(tree.children).length !== 0) {
    return Object.entries(tree.children).reduce((accumulator, currentValue) => {
      if (currentValue[0])
        currentValue[1].tags.forEach((tag) => {
...
const getKeywords = (pictoCategories, categoryTree) => {
  const tree = categoryTree.toJSON()
  const nodes = jp.nodes(tree, '$..keywords')
  const keywords = pictoCategories.reduce((accumulator, currentValue) => {
    const keywords = nodes
      .filter((node) => node.path[node.path.length - 2] === currentValue)
      .map((node) => node.value)
    return keywords[0] && keywords[0].length
...
const PictogramTags = ({
  ...
  const searchCategory = removeDiacritics(
    stopWords(searchText, locale)
  ).toLowerCase()
  const tags = pictograms.length
    ? getTags(searchCategory, categories, locale)
    : null
  let renderTags
  ...
  if (tags.length) {
    ...
    pictoTags.forEach((tag) => {
      if (
        pictograms.every((pictogram) => pictogram.tags.indexOf(tag) !== -1) ||
        selectedTags.has(tag)
      )
        pictoTags.delete(tag)
    })
    ...
    const pictoCategories = pictograms
      .filter((pictogram) =>
        pictogram.keywords.some(
          (keyword) =>
            searchCategory ===
            removeDiacritics(stopWords(keyword.keyword, locale)).toLowerCase()
        )
      )
    ...
    renderTags = keywords.map((keyword) => (
      <Chip
        style={styles.chip}
        key={keyword}
  ...
  return (
    <LanguageProvider messages={translationMessages} paramLocale={locale}>
      <div style={{ display: 'flex', flexWrap: 'wrap' }}>{renderTags}</div>
    </LanguageProvider>
  )
}
