...
class UpdateMaterialView extends PureComponent {
  ...
  renderContent() {
    ...
    const listActivities = activities
      ...
      .map((activity) => ({
        value: parseInt(activity.code, 10),
        text: formatMessage(filterMessages[activity.text]),
      }));

    const listAreas = areas
      .filter((area) => formData.areas.indexOf(area.code) !== -1)
      .map((area) => ({
        value: parseInt(area.code, 10),
        text: formatMessage(filterMessages[area.text]),
      }));
...
export default connect(
  mapStateToProps,
  mapDispatchToProps
)(userIsAdmin(injectIntl(UpdateMaterialView)));
