...
const ProfileIntro = ({ name, lastLogin, picture, role, isRtl, hasFacebook, hasGoogle, intl, pictureProvider, onPictureChange }) => (
  <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center' }}>
    <div style={{ marginRight: 20 }}>
      <img role='presentation' src={picture} alt={name} style={{ width: '230px', height: '230px' }} />
    </div>
    <div>
      <H3>
      ...
      >
        ...
        <MenuItem key='default' value={ARASAAC} primaryText={intl.formatMessage(messages['arasaacProfilePicture'])} />
        <MenuItem key='google' disabled={!hasGoogle} value={GOOGLE} primaryText={intl.formatMessage(messages['googleProfilePicture'])} />
        <MenuItem key='facebook' disabled={!hasFacebook} value={FACEBOOK} primaryText={intl.formatMessage(messages['facebookProfilePicture'])} />
      </SelectField>
