...
import { makeSelectName, makeSelectRole } from "containers/App/selectors";
...
class CollaboratorsView extends Component {
  ...
  render() {
    ...
    const localeCollaborators = filter
      ? collaborators.filter(
          (colaborator) => colaborator.languages.indexOf(locale) !== -1
        )
      : collaborators;
    return (
      <View left={true} right={true}>
        <ReadMargin>
          ...
          <RaisedButton
            label={
              filter ? (
                ...
                <FormattedMessage {...messages.filterByLanguage} />
              )
            }
            primary={true}
          ...
          {localeCollaborators.length ? (
            ...
            >
              {localeCollaborators.map((collaborator) => (
                <CollaboratorSnippet
            ...
            <div>
              <P>
                No hay traductores en este idioma. Puedes ponerte en contacto
                con nosotros para colaborar.
              </P>
            </div>
          )}
          {filter && (
